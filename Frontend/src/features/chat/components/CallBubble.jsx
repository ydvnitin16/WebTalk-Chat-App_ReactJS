...
const CallBubble = ({
  ...
  const getIcon = () => {
    ...
    if (status === "completed" && isMine)
      return <PhoneOutgoing size={16} className="text-[#fff]" />;
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium">{getCallLabel()}</span>

              <span
                className={`text-[10px] ${
                  isMine ? "text-zinc-200" : "text-zinc-500 dark:text-zinc-400"
                }`}
              >
                {getDuration()}
              </span>
